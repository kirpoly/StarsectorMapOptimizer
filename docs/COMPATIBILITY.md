...
последовательность только одна: exact-hash presentation-stage, затем structural-stage. Presentation
публикует owner/mask contract; structural-stage валидирует входной contract и обязан сохранить его
при любых собственных изменениях. Полностью отсутствующий presentation-state допустим при локальном
hash/loader skip, поэтому независимые structural patches продолжают применяться. Частичный или
чужой presentation-state считается конфликтом композиции и не модифицируется.
