...
Три exp6-блока выбраны по profiler/JFR-телеметрии; same-save повторный прогон подтвердил снижение
allocations, но также обнаружил exp6 CPU-регрессию в пустом `IdentityHashMap.clear()`. Structural
proof показывает только однозначность site, linkage, no-escape и verifier postconditions. Exp7
runtime regression отдельно проверяет пропуск пустого clear и сохранение cleanup заполненной карты;
величину итогового frame-time улучшения должен подтвердить новый campaign-прогон.
